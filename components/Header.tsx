...
function Header() {
  ...
  return (
   ...
   md:items-start md:space-x-6 px-2 pt-10 pb-5 md:p-10 md:pb-5">
   <Link href="/">
   <Image
...
<div className="w-full md:max-w-2xl">
  ...
  <form action={formData => {
    ...
    if (pages) params.set("min_price", minPrice.toString())
    if (pages) params.set("max_price", maxPrice.toString())
    router.push(`/search/${searchTerm}?${params.toString()}`)
  }}>
    <div className="flex items-center gap-2 w-full px-4">
      <div className="flex items-center space-x-2 bg-white
       shadow-xl rounded-full border-8 px-6 py-4 md:max-w-5xl flex-1">
    ...
    max-w-lg md:max-w-none mx-auto">
      <SearchSelect onValueChange={(value) => setPages(value)}
      className="min-w-4" placeholder="# number of pages">
        {[...Array(100)].map((_, i) => (
...
<div className="hidden lg:flex flex-1 justify-end">
  <Avatar name="Dayyan Khan" round size="50"/>
</div>
   </header>
